...
<?php 
	$query = "SELECT * FROM tblblotter ORDER BY id DESC";
...
<head>
	...
	<title>Blotter/Incident Complaint -  BESIS</title>
</head>
<body>
											...
											<tbody>
												<?php if(!empty($blotter)): ?>
													...
													<tr>
														...
														<td><?= ucwords($row['casenum']) ?></td>
														<td><?= ucwords($row['type']) ?></td>
														<td>
															<?php if($row['status']=='Scheduled'): ?>
																<span value="Scheduled" class="badge badge-warning">Scheduled</span>
															<?php elseif($row['status']=='Active'): ?>
																<span value="Active" class="badge badge-danger">Active</span>
															<?php else: ?>
																<span value="Settled" class="badge badge-success">Settled</span>
															<?php endif ?>
														</td>
														<?php if(isset($_SESSION['username'])):?>
														<td>
                                                        ...
                                                        <div class="form-button-action">
															<a type="button" href="#edit" data-toggle="modal" class="btn btn-link btn-primary check_casenum" 
																title="Edit Blotter" onclick="editBlotter1(this)" data-id="<?= $row['id'] ?>" data-complainant="<?= $row['complainant'] ?>" 
																data-respondent="<?= $row['respondent'] ?>" data-casenum="<?= $row['casenum'] ?>" data-type="<?= $row['type'] ?>" data-l="<?= $row['location'] ?>" 
                                                            ...
                                                            <a type="button" data-toggle="tooltip" href="generate_blotter_summon.php?id=<?= $row['id'] ?>" class="btn btn-link btn-primary" data-original-title="Generate Sumon">
																	<i class="fas fa-file-alt"></i>
																</a>
                            ...
                            <form method="POST" action="model/save_blotter.php" >
								...
								<div class="row">
									<div class="col-md-6">
										<div class="form-group">
											<label>Case No.</label>
											<input type="text" class="form-control" placeholder="Enter Case No." name="casenum" required>
										</div>
									</div>
									<div class="col-md-6">
										<div class="form-group">
                            ...
                            <form method="POST" action="model/edit_blotter.php" >
								...
								<div class="row">
									<div class="col-md-6">
										<div class="form-group">
											<label>Case No.</label>
											<input type="text" class="form-control" placeholder="Enter Case No." id="casenum" name="casenum" required>
										</div>
									</div>
									<div class="col-md-6">
										<div class="form-group">
