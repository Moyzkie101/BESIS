...
<head>
		...
		<title>Barangay Bolocboloc | CHANGE PASSWORD</title>
			...
			<div class="account-form">
				...
				<div class="account-form-inner">
					<div class="account-container">
						...
						<form class="contact-bx" method="POST" action="change_password.php">
							<div class="row placeani">
								<div class="col-lg-12">
									<div class="form-group">
										<div class="input-group">
											...
											<input class="form-control" name="email" value="<?php echo $_GET['email']?>" type="text" name="verification-code" maxlength="150" hidden>
										</div>
									</div>
								</div>
								<div class="col-lg-12">
								...
								<div class="col-lg-12 m-b30" align="center">
										<button name="submitted" type="submit" class="red-hover btn button-md" style="background-color: #5691cf!important;">Submit</button>
								</div>
							</div>
						</form>
					</div>
...
<?php
	...
	if(isset($_POST['submitted'])){
	 	...
	 	$mail = mysqli_query($conn,"select * from tblresident where email='".$_POST["email"]."'");

	 	$mail2 = mysqli_query($conn,"select * from tbl_users where email='".$_POST["email"]."'");

	 	if(mysqli_affected_rows($conn) > 0){
	 		$mailing = mysqli_fetch_array($mail);

	 		$mailing2 = mysqli_fetch_array($mail2);

	 		if(mysqli_affected_rows($conn) > 0){
	 	
				if ($_POST['password'] === $_POST['confirm-password']) {
			        mysqli_query($conn, "UPDATE tbl_users SET password='".sha1($_POST['password'])."', code='' WHERE email='".$mailing['email']."'");
			        
			        mysqli_query($conn, "UPDATE tbl_users SET password='".sha1($_POST['password'])."', code='' WHERE email='".$mailing2['email']."'");
